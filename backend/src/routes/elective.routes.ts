...
async function checkElectiveFormActive() {
  try {
    ...
    console.log('DEBUG checkElectiveFormActive:');
    ...
    console.log('  Found deadline record:', activeForm);

    if (activeForm) {
      console.log('  Start Date:', activeForm.start_date);
      console.log('  End Date:', activeForm.end_date);
      console.log('  Is between dates?', 
        currentDate >= activeForm.start_date && currentDate <= activeForm.end_date
      );
    }
    ...
    // Check date range
    if (currentDate >= activeForm.start_date && currentDate <= activeForm.end_date) {
      console.log('  ✅ Form is active!');
      return activeForm;
    } else {
      console.log('  ❌ Form is NOT in date range');
      return null;
    }
  } catch (error) {
...
router.get('/elective-courses', async (req: Request, res: Response) => {
  try {
    ...
    if (!activeForm) {
      console.log('DEBUG: /elective-courses - Form not active');
...
router.get('/student-electives/:studentId', async (req: Request, res: Response) => {
  try {
    ...
    console.log('DEBUG: /student-electives/:studentId - studentId:', studentId);
    console.log('DEBUG: activeForm:', activeForm ? 'FOUND' : 'NOT FOUND');
